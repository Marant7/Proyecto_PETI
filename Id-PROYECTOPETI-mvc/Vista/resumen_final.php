...
$empresa = $_SESSION['temp_empresa'] ?? [];
$visionMision = $_SESSION['temp_vision_mision'] ?? [];
$valores = $_SESSION['temp_valores'] ?? [];
$objetivos = $_SESSION['temp_objetivos'] ?? [];
$cadenaValor = $_SESSION['temp_cadena_valor'] ?? [];
$matrizBCG = $_SESSION['matriz_bcg'] ?? [];
$bcgFortalezas = $_SESSION['bcg_fortalezas'] ?? '';
$bcgDebilidades = $_SESSION['bcg_debilidades'] ?? '';
$fuerzasPorter = $_SESSION['fuerzas_porter'] ?? [];
$pestResultados = $_SESSION['pest_resultados'] ?? [];
...
<body>
    <div class="container">
        <h1>Resumen Final del Plan Estratégico</h1>
        <div class="section empresa">
            <h3>📋 Datos de la Empresa</h3>
            <p><strong>Nombre:</strong> <?= safe($empresa['nombre_empresa']) ?></p>
        ...
        <div class="section fuerzas-porter">
            ...
            <?php if (!empty($pestResultados)): ?>
                ...
                <div class="subsection">
                    ...
                    <ul>
                        <?php if ($pestResultados['factor1'] >= 15): ?>
                            <li><strong>Factor Demográfico:</strong> Aprovechar tendencias demográficas favorables para expandir mercados objetivo.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor2'] >= 15): ?>
                            <li><strong>Factor Legal/Político:</strong> Capitalizar estabilidad política y marcos regulatorios favorables.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor3'] >= 15): ?>
                            <li><strong>Factor Económico:</strong> Aprovechar condiciones económicas favorables para inversión y crecimiento.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor4'] >= 15): ?>
                            <li><strong>Factor Tecnológico:</strong> Utilizar avances tecnológicos para innovar y mejorar procesos.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor5'] >= 15): ?>
                            <li><strong>Factor Medioambiental:</strong> Aprovechar tendencias de sostenibilidad para diferenciación competitiva.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['total'] >= 100): ?>
                            <li><strong>Entorno favorable:</strong> Capitalizar el contexto positivo general para expansión estratégica.</li>
                        <?php endif; ?>
                        <?php if (count(array_filter([$pestResultados['factor1'], $pestResultados['factor2'], $pestResultados['factor3'], $pestResultados['factor4'], $pestResultados['factor5']], function($x) { return $x < 15; })) == 0): ?>
                            <li><strong>Posición sólida:</strong> Mantener ventajas competitivas en todos los factores del entorno.</li>
                        <?php endif; ?>
                    </ul>
                </div>
                ...
                <div class="subsection">
                    ...
                    <ul>
                        <?php if ($pestResultados['factor1'] < 10): ?>
                            <li><strong>Factor Demográfico:</strong> Cambios demográficos adversos pueden afectar la demanda de productos/servicios.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor2'] < 10): ?>
                            <li><strong>Factor Legal/Político:</strong> Inestabilidad política y cambios regulatorios pueden generar incertidumbre.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor3'] < 10): ?>
                            <li><strong>Factor Económico:</strong> Condiciones económicas desfavorables pueden impactar la rentabilidad.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor4'] < 10): ?>
                            <li><strong>Factor Tecnológico:</strong> Obsolescencia tecnológica puede reducir competitividad.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['factor5'] < 10): ?>
                            <li><strong>Factor Medioambiental:</strong> Presiones ambientales pueden aumentar costos operativos.</li>
                        <?php endif; ?>
                        <?php if ($pestResultados['total'] < 75): ?>
                            <li><strong>Entorno desafiante:</strong> Múltiples factores externos requieren estrategias de mitigación de riesgos.</li>
                        <?php endif; ?>
                        <?php if (count(array_filter([$pestResultados['factor1'], $pestResultados['factor2'], $pestResultados['factor3'], $pestResultados['factor4'], $pestResultados['factor5']], function($x) { return $x < 10; })) >= 3): ?>
                            <li><strong>Riesgo elevado:</strong> Múltiples factores críticos requieren atención inmediata.</li>
                        <?php endif; ?>
                    </ul>
                </div>
            <?php else: ?>
